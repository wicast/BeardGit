...
- **Pull every tag from a remote.** The Tags footer gains the other half of "sync my tags": "Pull All Tags" sits left of "Push All Tags" and reads the same remote picker, so pulling and pushing always name the same remote — the picker says so now ("Remote for tag push and pull", not "…push tags to"). It runs `git fetch <remote> --tags` as a cancellable task: every tag the remote advertises, and the objects those tags point at, come down while HEAD and the working tree stay as they were. The tag list and the graph refresh through the watcher's `refs_changed` event, the same path the toolbar's Fetch uses, and there is no success toast for the same reason tag push has none: the task drawer row reaching a terminal state is the confirmation.
...
- **The Tags footer keeps both sync buttons — and both look like buttons — whether or not the repository has tags.** "Pull All Tags" rendered as bare text: it used the `neutral` button variant, whose fill is `--bg-secondary`, which is the list panel's own surface, so it had no fill and a transparent border to be seen against. Pull is now the `success` variant, readable and distinct from the `primary` push beside it. The pair also stays on screen when there is nothing to list — an empty tag list is exactly when "pull the tags down from the remote" is the useful action, and the footer used to disappear with the rows. In that state "Push All Tags" is disabled with a "No tags to push" tooltip, since `git push --tags` has nothing to send, while pull and the remote picker stay live. `List` gained a `footerWhenEmpty` opt-in for this; footers that page through rows ("Load more") keep the old behaviour.
- **A tag whose name matches a branch wears the tag colour, not the branch colour.** The graph colours a ref badge by the ref's kind, which it reads from the name's prefix — but the commit walk handed it `shorthand()` names (`v1.0`, `origin/main`, `main`), stripping the very prefix it classifies by, so every tag fell through to the branch colour and `refs/tags/v1.0` was indistinguishable from a branch called `v1.0`. Refs are now fully qualified from `build_ref_map` down, and the kind/label parsing lives in one place (`utils/ref`) instead of being re-derived by each consumer. The graph layout cache schema is bumped (4 → 5): its key tracks repository state only, so a layout cached before this change would keep serving bare names until some ref happened to move.
...
- **The terminal keeps its command line straight when the app was launched from a non-terminal shell.** Spawned shells inherited the GUI process's environment untouched, and a BeardGit started from Finder, Spotlight, or an agent/IDE shell carries `TERM=dumb` (or no `TERM` at all). zsh then finds no cursor-forward capability and its line editor redraws by printing literal spaces: the first keystroke wiped the prompt (the spaces ran straight over it), suggestion ghost text landed at shifted columns, and typing scrambled the line into fragments. The emulator — not whatever happened to launch the app — decides what the child may assume about its terminal: spawned shells now get `TERM=xterm-256color` and `COLORTERM=truecolor` unless a caller supplies a real `TERM`, and `dumb` is overridden the same way.
- **Reopening a terminal tab no longer shows an empty pane, and typing no longer reaches dead sessions.** The interactive xterm pool recycled instances that had already been opened into a destroyed container — xterm cannot re-open an opened terminal, so the next mount rendered nothing while every keystroke listener from the instance's earlier lives kept firing. Opened instances are now disposed on release; only never-opened warm spares are reused. The interactive terminal also stops rewriting bare newlines into carriage-return pairs (a live PTY speaks CRLF itself), and the read-only job-log pool had the same recycle-and-vanish defect, now fixed the same way. Web-font timing no longer skews column widths: the first fit measured the fallback font before Fira Code loaded and nothing ever re-measured, so refits now follow the browser's font-loading events.
